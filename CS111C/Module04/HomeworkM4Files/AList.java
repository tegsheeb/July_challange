...
public class AList<T extends Comparable<? super T>> implements ListInterface<T>, Comparable<AList<T>> {
	...
	public int removeAll(T element) {
		int count= 0;
		AList<Integer> indexOfRemovingItems = new AList<Integer>();
		
		for (int i = 1; i <= numberOfEntries; i++) {
			if(list[i].equals(element)) {
				count++;
				indexOfRemovingItems.add(i);
			}
		}

		for (int i = indexOfRemovingItems.getLength(); i > 0; i--) {
			remove(indexOfRemovingItems.getEntry(i));
		}
	...
	public boolean equals(Object other) {
		if(other instanceof AList<?>) {
			AList<T> otherAList = (AList<T>) other;
			if(numberOfEntries != (otherAList.getLength())) {
				return false;
			} else {
				for(int i = 1; i <= numberOfEntries; i++) {
					if(!list[i].equals(otherAList.getEntry(i))){
						return false;
					}
				}
				return true;
	...
	public int compareTo(AList<T> otherList) {
		if(numberOfEntries == otherList.getLength()) {
			for(int i = 1; i <= numberOfEntries; i++) {
				if(list[i].compareTo(otherList.getEntry(i)) != 0) {
					return list[i].compareTo(otherList.getEntry(i));
				}
			}
			return 0;
		} else if (numberOfEntries > otherList.getLength()){
			return 1;
		} else {
			return -1;
		}
