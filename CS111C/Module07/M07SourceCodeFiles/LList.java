...
public class LList<T extends Comparable<? super T>> implements ListInterface<T> {
	...
	private boolean contains(T anEntry, Node current) {
		/* option 1: multiple return statements */
		/*
		if(current==null) { // base case
			return false;
		} else { // recursive case
			return current.data.equals(anEntry) || contains(anEntry, current.next);
		}
		*/
		...
		if(current==null) { // base case
			...
			result = current.data.equals(anEntry) || contains(anEntry, current.next);
		}
		return result;
	}
